...
class SupplyScreen(MDScreen):
    ...
    def save(self):
        ...
        id = self.get_supply_id()

        query = "UPDATE Поставка1 SET idПоставщика=%s, idАвтомобиля=%s,  Количество=%s, Дата=%s WHERE id=%s"
        values = [shipper_id, id_auto, int(count.text), cur_date, id]
...
class SupplyApp(MDApp):
    def build(self):
        ...
        return Builder.load_file("edit_supply_window.kv")
