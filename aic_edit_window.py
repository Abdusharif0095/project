...
class AutoScreen(MDScreen):
    ...
    def save(self):
        ...
        query = "UPDATE Автомобиль SET idМарки=%s, Категория=%s, ГодВыпуска=%s, КаробкаПередач=%s, Пробег = %s, Цвет = %s, РасходТопливо = %s, Цена = %s, Количество = %s WHERE id=%s"
        values = [int(mark.text), category.text, int(year.text), transmission.text, int(mileage.text), color.text, int(fuel.text), int(price.text), int(count.text), id]
        print(query, values)
...
class AutoApp(MDApp):
    def build(self):
        ...
        return Builder.load_file("aic_edit_window.kv")
