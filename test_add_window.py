...
class TestScreen(MDScreen):
    ...
    def save(self):
        ...
        for row in data:
            if int(row[0]) == id:
                continue
            if client_id == row[1] and cur_date == row[5] and cur_time == row[4] and employee_id == row[
                2] and auto_id == row[3]:
        ...
        query = "SELECT MAX(id) from Тест;"
        data = get_data.get_data_from_dt(query)
        idd = int(data[0][0]) + 1

        query = "INSERT INTO Тест VALUES(%s, %s, %s, %s, %s, %s);"

        values = [idd, client_id, employee_id, id_auto, cur_time, cur_date]
...
class TestApp(MDApp):
    def build(self):
        ...
        return Builder.load_file("test_add_window.kv")
