...
class OrderScreen(MDScreen):
    ...
    def save(self):
        ...
        query = "UPDATE Заказ SET idКлиента=%s, idАвтомобиля=%s, Предоплата=%s WHERE id=%s"
        values = [client_id, id_auto, int(prepeymant.text), id]
    ...
    def delete(self):
        id = self.get_order_id()
        query = "DELETE FROM Заказ WHERE id=%s;"
        values=[id]

        if get_data.delete(query, values):
            return 1
        else:
            self.ids["label"].text = "Предзаказ \n\n Что то пошло не так!"
...
class OrderApp(MDApp):
    def build(self):
        ...
        return Builder.load_file("order_edit_window.kv")
